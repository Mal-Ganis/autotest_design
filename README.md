...
本文件面向**刚接触命令行、Python 或本项目的同学**，尽量把每一步写清楚。若你已有经验，可直接看 **「4. 第一次跑通（复制粘贴版）」** 和 **「5. 目录与文件是干什么的」**。当前流水线已实现 **S1～S4**（P1：摄入与结构化；P2：风险与覆盖项）。
...
**本 README 覆盖「AutoTestDesign 工具」里已实现的 P1（需求摄入 + 结构化）与 P2（风险评分与覆盖项识别）。**  
...
按顺序执行 **4.2 → 4.3 → 4.5 → 4.6**，或使用下面等价的一条龙（仍在项目根目录）：
...
├── README.md                    ← 本说明（你正在读）
├── requirements.txt             ← Python 依赖列表（S1～S4 当前以标准库为主）
├── contracts/
...
**下游：** P3 从 **`04_coverage_items.json`** 继续生成 **`05_strategies.json`** 等（见分工文档）。
...
- **P3**：读 **`04_coverage_items.json`**，编写策略与黑盒生成；请在 `SCHEMA.md` **追加** S5～S6，勿删已有字段。
- **P4**：可追溯、交互审查、导出、`launcher.py`；串联时在 P2 之后接上。
...
**与工具链的关系**：可用 `scripts/` 产出的 `03_with_risk.json`、`04_coverage_items.json` 支撑 FR 2.0 与测试优先级描述；详细用例与执行文档由全组按作业「Mainly」段落继续补充。
